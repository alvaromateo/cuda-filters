...
int main(int argc, char **argv) {
    ...
    if(nChannels == 3){ //rgb
        ...
        if (image.isContinuous()) {
            ...
            for (int i = 0; i < image.rows; ++i) {
                blue.insert(blue.end(), bgr[0].ptr<uchar>(i), bgr[0].ptr<uchar>(i)+bgr[0].cols);
                green.insert(green.end(), bgr[1].ptr<uchar>(i), bgr[1].ptr<uchar>(i)+bgr[1].cols);
                red.insert(red.end(), bgr[2].ptr<uchar>(i), bgr[2].ptr<uchar>(i)+bgr[2].cols);
            }
        }
    }
	...
	//TODO: nom fitxer output --> nomoriginal_filtre.png
